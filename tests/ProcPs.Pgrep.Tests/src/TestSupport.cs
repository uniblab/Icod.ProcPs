/*
	Icod.ProcPs.Pgrep.Tests
	Tests for the pgrep command implementation.
...
internal sealed class FakeControl : IProcMatchControl {
	...
	/// <summary>Gets handler pids.</summary>
	internal HashSet<int> HandlerPids { get; } = [];
	...
	public ProcessOperationResult<ProcessSignalDisposition> ObserveDisposition( ProcProcessSnapshot process, ProcessSignal signal )
		=> ProcessOperationResult<ProcessSignalDisposition>.Success(
			this.HandlerPids.Contains( process.ProcessId ) ? ProcessSignalDisposition.Caught : ProcessSignalDisposition.Default
		);
