...
""" + "\n";
	...
	public static async Task<int> RunAsync(
		...
		try {
			while ( true ) {
				...
				if ( !memory.HasValue )
					return await ReportMemoryFailureAsync( stderr, memory, cancellationToken ).ConfigureAwait( false );
				...
				if ( remaining.HasValue ) {
					...
					break;
				if ( !parsed.Line )
					await WriteLineAsync( stdout, string.Empty, cancellationToken ).ConfigureAwait( false );
	...
	private static async Task<int> ReportMemoryFailureAsync( Stream? stderr, ProcObservedValue<ProcMemoryInfo> observation, CancellationToken cancellationToken ) {
		string text;
		if ( ProcObservationAvailability.Unsupported == observation.Availability )
			text = "free: memory information with procps-ng semantics is not available on this platform";
		else if ( ProcObservationAvailability.Unavailable == observation.Availability && ( observation.Diagnostic?.Contains( "meminfo", StringComparison.OrdinalIgnoreCase ) ?? false ) )
			text = "free: Memory information file /proc/meminfo does not exist";
		else
			text = "free: Unable to create meminfo structure";
		if ( ProcObservationAvailability.Unsupported == observation.Availability && !string.IsNullOrWhiteSpace( observation.Diagnostic ) )
	...
	private static string Render( ProcMemoryInfo memory, FreeArguments options ) {
		...
		if ( options.Wide )
			AppendRow( builder, "Mem:", F( values.Total ), F( values.Used ), F( values.Free ), F( values.Shared ), F( values.Buffers ), F( values.Cache ), F( values.Available ) );
		else
			AppendRow( builder, "Mem:", F( values.Total ), F( values.Used ), F( values.Free ), F( values.Shared ), F( SaturatingAdd( values.Buffers, values.Cache ) ), F( values.Available ) );
		if ( options.LoHi ) {
		...
		if ( options.Total )
			AppendRow( builder, "Total:", F( SaturatingAdd( values.Total, values.SwapTotal ) ), F( SaturatingAdd( values.Used, values.SwapUsed ) ), F( SaturatingAdd( values.Free, values.SwapFree ) ) );
		if ( options.Committed )
			AppendRow( builder, "Comm:", F( values.CommitLimit ), F( values.Committed ), F( unchecked(values.CommitLimit - values.Committed) ) );
		return builder.ToString();
	}
	private static void AppendRow( StringBuilder builder, string label, params string[] fields ) {
		...
		for ( var index = 0; index < fields.Length; index++ ) {
			if ( 0 < index )
				builder.Append( ' ' );
			builder.Append( fields[ index ].PadLeft( 11 ) );
		}
	...
	public static string FormatSize( ulong bytes, int exponent = 0, bool si = false, bool human = false ) {
		...
		if ( !human ) {
			...
			if ( 0 == exponent )
				return ( bytes / ( si ? 1000UL : 1024UL ) ).ToString( CultureInfo.InvariantCulture );
	...
	private static MemoryValues Derive( ProcMemoryInfo memory ) {
		...
		if ( 0 == available || available > total )
			available = free;
	...
	private static FreeArguments ParseArguments( string[] args ) {
				...
				if ( option is "seconds" or "count" ) {
					if ( null == value ) {
						if ( index + 1 >= args.Length )
							return Fail( $"free: option '--{option}' requires an argument", true );
						value = args[ ++index ];
					}
					var failure = ApplyValueOption( option, value );
					if ( null != failure )
						return failure;
					continue;
				}
				if ( null != value )
					return Fail( $"free: option '--{option}' doesn't allow an argument", true );
				var immediate = ApplyFlagOption( option );
				if ( null != immediate )
					return immediate;
				continue;
			}
			for ( var position = 1; position < token.Length; position++ ) {
				...
				if ( option is 's' or 'c' ) {
					...
					if ( position + 1 < token.Length )
						...
						if ( index + 1 >= args.Length )
							return Fail( $"free: option requires an argument -- '{option}'", true );
						value = args[ ++index ];
					}
					var failure = ApplyValueOption( 's' == option ? "seconds" : "count", value );
					if ( null != failure )
						return failure;
					break;
				}
				...
				if ( null == name )
					return Fail( $"free: invalid option -- '{option}'", true );
				var immediate = ApplyFlagOption( name );
				if ( null != immediate )
					return immediate;
			}
		}
		...
		FreeArguments? ApplyValueOption( string option, string value ) {
			if ( "seconds" == option ) {
				if ( !TryParseSeconds( value, out var seconds ) )
					return Fail( $"free: seconds argument failed: '{value}'", false );
				if ( 0d >= seconds )
					return Fail( $"free: seconds argument `{value}' is not positive number", false );
				var microseconds = (float)( seconds * 1_000_000d );
				if ( 1f > microseconds || !float.IsFinite( microseconds ) || microseconds > long.MaxValue / 10f )
					return Fail( $"free: seconds argument `{value}' is not positive number", false );
				interval = TimeSpan.FromTicks( checked((long)microseconds * 10L) );
				repeat = true;
				return null;
			}
			if ( !int.TryParse( value, NumberStyles.AllowLeadingWhite | NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture, out var parsedCount ) || 1 > parsedCount )
				return Fail( $"free: failed to parse count argument: '{value}'", false );
			count = parsedCount;
			repeat = true;
			return null;
		}
		FreeArguments? ApplyFlagOption( string option ) {
			if ( option is "bytes" or "kibi" or "mebi" or "gibi" or "tebi" or "pebi" or "kilo" or "mega" or "giga" or "tera" or "peta" ) {
				if ( unitSet )
					return Fail( "free: Multiple unit options don't make sense.", false );
				unitSet = true;
				exponent = option switch {
					"bytes" => 1,
					"kibi" or "kilo" => 2,
					"mebi" or "mega" => 3,
					"gibi" or "giga" => 4,
				...
				if ( option is "kilo" or "mega" or "giga" or "tera" or "peta" )
					si = true;
				return null;
			}
			switch ( option ) {
				...
				case "wide":
					...
					break;
				case "help":
					return new( exponent, si, human, lohi, line, total, committed, wide, repeat, count, interval, true, false, null, false );
				case "version":
					return new( exponent, si, human, lohi, line, total, committed, wide, repeat, count, interval, false, true, null, false );
			}
			return null;
		}
	...
	private static LongOptionResolution ResolveLongOption( string name, string token ) {
		...
		if ( 1 < matches.Length )
			return new( null, $"free: option '{token}' is ambiguous; possibilities: {string.Join( " ", matches.Select( option => $"'--{option}'" ) )}" );
	...
	private static bool TryParseSeconds( string text, out double value ) {
		...
		if ( integerEnd > integerStart && !double.TryParse( text[ integerStart..integerEnd ], NumberStyles.None, CultureInfo.InvariantCulture, out value ) )
			return false;
		if ( index == text.Length ) {
		...
		var multiplier = 0.1d;
		while ( index < text.Length && char.IsAsciiDigit( text[ index ] ) ) {
			fraction += ( text[ index ] - '0' ) * multiplier;
			multiplier /= 10d;
			index++;
		}
		if ( index != text.Length )
			return false;
		if ( integerEnd == integerStart && 0d == fraction ) {
			value = 0d;
			return true;
		}
		value += fraction;
